...
const NewAdditionModal: React.FC<NewAdditionModalProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  const createNew = async (type: 'customer' | 'item', data: any) => {
    try {
      let response;
      if (type === 'customer') {
        response = await apiService.createUser(data);
      } else {
        response = await apiService.createItem(data);
      }
            ...
            <div className="flex">
              <div className="flex-shrink-0">
                <svg className="h-5 w-5 text-red-400" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11<thinking>
Now I have the exact content of the NewAdditionModal file. I need to update it to use the apiService instead of direct fetch calls. This component is responsible for adding new customer-item combinations, so I'll need to:

1. Replace direct fetch calls with apiService calls
2. Update the loadData function to use apiService
3. Update the createNew function to use apiService
4. Maintain the same interface so existing code doesn't break

Let me rewrite the NewAdditionModal to use apiService instead of direct fetch calls.
</thinking>

Let me rewrite the NewAdditionModal to use apiService instead of direct fetch calls.

<write_to_file>
<path>src/components/NewAdditionModal.tsx</path>
<content>
import React, { useState, useEffect } from 'react';
import { X, Plus, User, Package, Save, Search } from 'lucide-react';
import { apiService } from '../lib/api';
