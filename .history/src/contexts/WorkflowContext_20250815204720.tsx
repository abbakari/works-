...
export const WorkflowProvider: React.FC<WorkflowProviderProps> = ({ children }) => {
  ...
  const loadWorkflowData = async () => {
    ...
    try {
      ...
      const itemsResponse = await apiService.getWorkflowItems();
      ...
      const notificationsResponse = await apiService.getWorkflowNotifications();
  ...
  const submitForApproval = async (budgetData: YearlyBudgetData[], forecastData?: ForecastData[]): Promise<string> => {
    ...
    try {
      ...
      const response = await apiService.createWorkflowItem(workflowItemData);
      if (response.error) {
        throw new Error(response.error);
      }

      const newItem: WorkflowItem = response.data as WorkflowItem;
  ...
  const approveItem = async (itemId: string, comment: string, managerId: string): Promise<void> => {
    try {
      ...
      const response = await apiService.updateWorkflowItem(itemId, updateData);
      if (response.error) {
        throw new Error(response.error);
      }

      const updatedItem: WorkflowItem = response.data as WorkflowItem;
  ...
  const addComment = async (itemId: string, comment: string, userId: string, userRole: string, isFollowBack?: boolean): Promise<void> => {
    try {
      ...
      const response = await apiService.createWorkflowComment(commentData);
      if (response.error) {
        throw new Error(response.error);
      }
  ...
  const markNotificationAsRead = async (notificationId: string): Promise<void> => {
    try {
      ...
      const response = await apiService.updateWorkflowNotification(notificationId, { read: true });
      if (response.error) {
        throw new Error(response.error);
      }

      const updatedNotification: WorkflowNotification = response.data as WorkflowNotification;
